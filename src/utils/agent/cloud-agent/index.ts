...
export async function createCloudAgentClient(
  ...
  return {
    ...
    dids: {
      ...
      prism: {
        list: async () => {
          // The registrar returns at most `limit` DIDs per request, so walk the
          // pages until one comes back short of a full page.
          const pageSize = 100;
          ...
          for (let offset = 0; ; offset += pageSize) {
            ...
            for (const managed of contents) {
              // A published DID is identified by its canonical form. An
              // unpublished one only resolves through its long form, which is
              // also what create returns.
              const value =
                managed.status === 'PUBLISHED'
                  ? managed.did
                  : (managed.longFormDid ?? managed.did);
              dids.push(Domain.DID.fromString(value));
            }
