...
import { Agent, Domain, ProtocolType } from '@hyperledger/identus-sdk';
...
describe('PeerDIDProvider', () => {
    ...
    test('create() calls agent.createPeerDID and stores the DID', async () => {
        const { result } = renderHook(
            () => ({
                db: useDatabase(),
                peer: usePeerDID(),
        ...
        const createSpy = vi.spyOn(agent, 'createPeerDID').mockResolvedValue(TEST_PEER_DID);
...
describe('PrismDIDProvider', () => {
    ...
    test('create() calls agent.createPrismDID', async () => {
        ...
        const createSpy = vi.spyOn(agent, 'createPrismDID').mockResolvedValue(prismDID);
