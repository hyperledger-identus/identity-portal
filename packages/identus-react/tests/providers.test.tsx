...
describe('DatabaseProvider', () => {
    ...
    test('setMediator(null) clears mediator', async () => {
        ...
        let fetchedMediator: Domain.DID | null = did;
        await act(async () => {
            fetchedMediator = await result.current.getMediator();
        });
...
describe('createResolver', () => {
    test('creates a resolver class with method "prism"', () => {
        const method = createResolver('https://resolver.example.com');
        expect(method.method).toBe('prism');
        expect(method.resolver.method).toBe('prism');
    });
    ...
    test('resolve() calls fetch with correct URL', async () => {
        ...
        const method = createResolver('https://resolver.example.com/');
        await method.resolver.resolve('did:prism:abc');
    ...
    test('resolve() throws on non-ok response', async () => {
        ...
        await expect(method.resolver.resolve('did:prism:abc')).rejects.toThrow(
